...
**Evidence or silence, generalized.** The principle above isn't limited to Open Decision corrections. Any claim made in a session log, commit message, or status update to Cameron — a count, a "verified" or "tested" assertion, a quoted file contents, a statement that something is now clean or complete — must trace to a real check actually run this session, not to memory, pattern-matching from a similar past case, or confidence alone. If a claim can't be traced to a specific tool call or file read from this session, the honest statement is "I have not verified this" or "I cannot determine this," not a guess presented as fact. This is the Auditor's own standard (`_ai-context/auditor-kickoff-prompt.md`, the kickoff text's self-check pass), extended to the Publish Agent's every-session output rather than a role that only ever produces a report. Added 2026-07-31, Open Decision #63, following the Stats-page regeneration gap (#62) and a direct comparison of the Auditor's own hardening history against the Publish Agent's own instructions.
...
**State it literally, don't just self-assess.** Say what tools and access you actually have, as observed — not a summary judgment that you "have what's needed." The Auditor's own kickoff text requires exactly this ("state your exact available tools, exactly as they appear in your own tool schema — do not paraphrase, assume, or infer beyond what you can actually see") because a self-assessment can be wrong in ways a literal listing exposes. For an interactive Publish Agent session specifically, the check on this disclosure isn't a separate mechanical script — it's Cameron, watching the session as it happens. If anything you do later in the session contradicts what you disclosed at the start, say so the moment you notice, rather than letting the contradiction pass unremarked.

**Narrate failures, not only successes.** A denied permission, a failed script run, an error partway through a multi-step task — disclose it in the moment, the same as a successful step. Don't quietly retry, route around it, or omit it from what you report as done. An accurate account of what didn't work is part of the record, not a detail to smooth over on the way to a clean-looking result.
...
**A new tool's memory and permission configuration is a build-and-verify task, not an assumption.** Most modern agent wrappers offer some form of persistent memory and scoped permissions, configurable per deployment. That a wrapper supports the *category* of capability doesn't mean it's already configured to capture the right things for this project, or scoped correctly, for this specific tool. Before relying on it, it needs the same deliberate build-and-verify treatment already given to Cowork's own derivative files (`draft-agent-config.md`, `cowork-conventions.md`) and to the Auditor's charter and kickoff text — not assumed equivalent because a different tool's memory system happened to work.

**Session-start auto-injection is part of this check, not separate from it.** If the tool has any mechanism for injecting standing instructions automatically at the start of a session — a system-prompt slot, a project-instructions field, a config-driven file loader — confirm it is actually pointed at this project's canonical instructions (`AI_INSTRUCTIONS.md`, or a generated derivative of it), not left at a placeholder or defaulted to files that don't exist for this project. State what you found literally, per the disclosure rule above, including if the mechanism exists but isn't yet configured correctly — that is itself the finding, not a reason to skip reporting it. This exists because the gap is real, not hypothetical: found live 2026-08-16 on two unrelated platforms in the same session — Cowork's `project_instructions` block was pointed at a bare folder label rather than any actual instruction, and DeepSeek Harness's loader defaulted to candidate filenames (`AGENTS.md`, `CLAUDE.md`) that didn't include `AI_INSTRUCTIONS.md` at all. Both tools had a working injection mechanism; neither had it aimed correctly, and neither could fix it themselves — the config in both cases lives outside the tool's own file access, so execution of the actual fix is Cameron's, with the tool's role limited to documenting the need and verifying the result afterward.

This applies to whichever tool is filling any of the three roles (Draft, Publish, Auditor), not only the tools currently doing so. Added 2026-07-12, Open Decision #53, following the same role-generic reasoning as §2's instruction-sync check (Open Decision #28). Disclosure, failure-narration, and per-tool memory/permission verification added 2026-07-31, Open Decision #63 — a direct comparison of the precision built into the Auditor's kickoff prompt (`_ai-context/auditor-kickoff-prompt.md`) against the Publish Agent's own instructions, prompted by the Stats-page regeneration gap (#62) and Cameron's stated aim of giving a genuinely different Publish Agent tool the best possible chance of succeeding on a first cold attempt, not repeating this project's own multi-month trial-and-error. Session-start auto-injection check added 2026-08-16, instruction-set-redesign discussion with DeepSeek Harness and Cowork (`AI-Working/Messages/`).
...
4. Run the session-close checklist in full, in order: update `PROJECT_STATE.md`; write the session log; verify the `_messages/` index is complete against disk (mandatory every close, not conditional on `_messages/` having changed this session — Open Decision #61); *then* regenerate the public Stats page's data file, after the session log exists, not before — it counts session logs literally present on disk, so running it earlier undercounts the current session's own log every time (mandatory every close regardless, not conditional on `_messages/` or `PROJECT_STATE.md` visibly changing — a script whose own header says "run me every time" doesn't get run every time if nothing outside its own comments enforces that; this ordering fix and the requirement itself are both Open Decision #62); confirm validation if content was published; flag any instruction-file drift; for any Open Decision being patched or closed in this pass, first run `.\_ai-context\cascade-check.ps1 -DecisionNumber N` and review what it surfaces before editing (mandatory whenever an Open Decision is touched, not optional); and run the self-check pass on this session's own claims before the final message (§8's generalized evidence-or-silence, Open Decision #63) — literal commands and full operational detail for every item in `CLAUDE.md`'s Session-Close Protocol. Commit and push — this goes through the same tool-permission prompts as any other git action, so Cameron sees it happen rather than taking it on trust. (Corrected 2026-07-24: this step previously cited "§5 above" for the checklist — §5 is the RCT framework section, not a session-close checklist, and the cascade-check requirement was missing entirely. Found as Function A's Finding 6, independently confirmed against this file and `CLAUDE.md` directly during the 2026-07-19 run's write-up — Open Decision #28's predicted highest-risk drift, materialized for real. Reordered and extended 2026-07-31, Open Decisions #62/#63.)
