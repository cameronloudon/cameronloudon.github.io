...
Career chronology and voice-and-tone direction for these pages live in `_ai-context/work-history-reference.md`, not here — moved 2026-08-16 during the instruction-set-redesign portability audit. This is Cameron's own biographical content, not Claude-Code-specific operational scaffolding, so it belongs in the tool-agnostic reference layer any Publish Agent tool can reach, not in this file.
...
4. Run `.\_ai-context\generate-stats-data.ps1` and commit the resulting `_data/stats.json` update. Mandatory at every session close, not conditional on whether `_messages/` or `PROJECT_STATE.md` visibly changed — the public `/stats/` page reads this file directly and goes stale silently otherwise. **Must run after step 3, not before** — `sessions_logged` counts `_session-logs/*.md` files literally present on disk, so running this before the new log exists undercounts it by one, every time, silently. This step originally sat under step 2, before the session log was written — the ordering bug this note exists to prevent. The script's own `.NOTES` said "run at session-close" from the day it was built, but nothing outside its own comments enforced that until Open Decision #62 (found 2026-07-31, Cowork's session-70 report); the ordering error itself was caught the same day it was introduced, by applying the same claim-verification discipline step 7 below now requires of every session, not just this one
5. If content was published in this session, confirm the validation checklist in `_ai-context/rct-validation-checklist.md` was run
6. Sync check: if you made changes that affect AI_INSTRUCTIONS.md or CLAUDE.md conventions, flag it to Cameron — do not let the two files drift
7. **Self-check pass on this session's own claims**, before sending the final message below (Open Decision #63). Scan the session log you just wrote, and anything else stated to Cameron this session, for: a commit hash cited without confirming it via `git log`/`git show`; a file path claimed to contain something specific without a fresh read of that file this session; a count or total stated as fact without a mechanical recount actually run; or a "tested against X" claim with no cited evidence the test actually ran. Correct or hedge anything that fails this check before proceeding. This is not a new standard invented for the Publish Agent — it is the same pre-save self-check discipline the Draft Agent already carries under `draft-agent-config.md`, applied symmetrically rather than left asymmetric
8. Commit `PROJECT_STATE.md`, the session log, and the regenerated `_data/stats.json`/`_ai-context/stats-history.json` in the same commit or immediately after the content commit, then push. This goes through the normal tool-permission prompts, same as any other git action — Cameron sees it happen rather than taking it on trust.
9. Say to Cameron: **"Session-X finalised — ready for the PR."** Only this message clears Cameron to create the PR.
