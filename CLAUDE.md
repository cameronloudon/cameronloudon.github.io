...
2. Update `PROJECT_STATE.md`:
   ...
   - Run `.\_ai-context\generate-stats-data.ps1` and commit the resulting `_data/stats.json` update. Mandatory at every session close, not conditional on whether `_messages/` or `PROJECT_STATE.md` visibly changed — the public `/stats/` page reads this file directly and goes stale silently otherwise. The script's own `.NOTES` said "run at session-close" from the day it was built, but nothing outside its own comments enforced that until this line existed (found 2026-07-31, Cowork's session-70 report — the #61-shaped gap recurred one script later)
