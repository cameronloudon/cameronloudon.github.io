...
- **`role:`** — the function being performed. One of: `Draft`, `Publish`, `Review`, `Audit`, `Human-in-the-loop`, `Candidate`. Not a tool, not an entity-type — "Human-in-the-loop" names a *kind of participation*, not an outcome, which is why it isn't "Decision." `Candidate` marks a participant not yet assigned a functional role, pending consensus — used only until a real role is agreed, never a standing designation. A message with `role: Candidate` should be revisited once the participant's role is settled; the value doesn't get backfilled on old files (same forward-only precedent as every other field in this schema). Added 2026-08-16, surfaced live by DeepSeek Harness having no enum value to use for itself while under evaluation (`AI-Working/Messages/`, instruction-set-redesign thread).
