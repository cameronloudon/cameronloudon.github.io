...
**Relocation is Cameron's hand only, added 2026-07-26 (Open Decision #47).** Since `auditor-kickoff-prompt.md`'s trailing structured summary block, a finding may arrive with its `type`/`title`/`timestamp`/`severity`/`failure-kind`/`refs` values already authored, but as the report's own final section rather than as literal frontmatter at the top. Moving that block to the top so it functions as real frontmatter is part of the same manual-relay step described above, done by Cameron when staging the copy in `AI-Working/Audit/` — never by the Publish Agent or the Draft Agent, both of whom are audited parties. This extends the never-edit rule above from "never reword or restructure a finding's content" to "never relocate it either," even for a mechanical, seemingly-safe cut-and-paste. If a staged file arrives with the block still trailing, unmoved, the correct response is to flag it back to Cameron and wait — not to fix it silently. Full deliberation and Cowork's independent agreement: `AI-Working/Messages/ccode-to-cowork-2026-07-26-kickoff-hardening-proposal.md` and its response.
