...
**Status:** Fourth version. The second version (below) was used the same day for the actual confirming run required to close Open Decision #47 — the first run ever conducted under it. Process integrity held on every front the hardening was built for: no self-referential claims, no fabricated quotation, no tool-call-limit fabrication, and the trailing block landed correctly once staged. Two mechanical gaps surfaced in the same run, both real findings and neither a process-integrity failure by the model: (1) the trailing block's first attempt collapsed into a single run-on line rather than genuine multi-line YAML — caught by Cowork (consulted for the correct generic YAML syntax, not for editing the finding itself), corrected by Cameron directly in the staged file per his own manual-relay role; Round 6 (below) makes the multi-line requirement explicit. (2) the model's `refs:` were written relative to the repo root, but the promotion destination (`_audit-findings/`) needs paths relative to itself — all six failed to resolve until Cameron corrected them the same way. Round 7 (below) states the actual required mapping explicitly. Open Decision #47 closed on this basis — see `_ai-context/decisions-archive.md`.

**Second version, 2026-07-26 (used for the confirming run above):** The 2026-07-24 text (below, "First version") was used for a repeatability test on 2026-07-26: three attempts, two rounds of live hardening between them, before a clean third run. That clean run itself then surfaced a fourth gap (a fabricated quotation) the hardening hadn't targeted yet. All four additions from that day — self-check pass, tool-call-limit fallback, the trailing summary block, and the quotation-accuracy re-scan — are folded in below, full propose→independent-review→consensus with Cowork, Cameron deciding the one open design question (making the quotation check mechanical, matching the self-check pass's own structure, rather than a framing rule alone). See Open Decision #47 for the full record; `AI-Working/Messages/ccode-to-cowork-2026-07-26-kickoff-hardening-proposal.md` and its response for the deliberation.
...
> failure-kind:
> refs:
>   - [each path relative to `_audit-findings/`, since that is where this report will be promoted — not relative to the repo root. A repo-root file like `AI_INSTRUCTIONS.md` becomes `../AI_INSTRUCTIONS.md`; a file already one level down like `_ai-context/auditor-charter.md` becomes `../_ai-context/auditor-charter.md`. One per line.]
> ---
> ```
>
> This closing block exists so whoever promotes your report can lift these values directly rather than re-deriving them from prose — it is not a request to restructure anything above it, and does not replace any report content you would otherwise produce. **This must be genuine multi-line YAML** — each field, and each `refs:` list item, on its own line, exactly as shown above. Do not collapse it into a single run-on line with the field names and dashes embedded as plain text; that is not valid YAML and cannot be used as frontmatter.
...
**Explicit multi-line YAML requirement, 2026-07-26.** The trailing-block addition's first-ever live use (the confirming run for Open Decision #47) produced a real report and real findings, but the closing block itself was collapsed into one run-on line with the field names and dashes embedded as plain text — not valid YAML, and not usable as frontmatter without a manual fix. The fenced example in the addition above was accurate but apparently not sufficient on its own to prevent this on a first attempt. This addition states the requirement in words as well as by example.

**Explicit refs path mapping, 2026-07-26.** The same confirming run wrote its `refs:` entries relative to the repo root (e.g. `AI_INSTRUCTIONS.md`), following the template's ambiguous "repo-relative path" wording literally and reasonably. But `generate-links-footer.ps1` resolves refs relative to wherever the promoted file itself lives — `_audit-findings/`, one level below root — so all six failed to resolve, and the finding's Links footer couldn't be generated until Cameron manually corrected the paths. Since the promotion destination is always the same, known folder, the fix is to state the actual required mapping explicitly rather than leave it to be inferred from an ambiguous adjective.
