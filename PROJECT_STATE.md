...
**Updated by:** Claude Code (Publish Agent) — Open Decisions bookkeeping cleanup: corrected two stale entries (#16 live test already ran, #19 already resolved via #21) and formalized two previously-untracked items (#31 `opencode.json` model, #32 duplicate session-start prompts) that had only ever existed in conversation and audit documents
...
`NEXT_SESSION_LOG_ID: 2026-07-05-016`

This session used `session-2026-07-05-015`.

The twenty-five existing session logs are: `session-2026-06-10-001`, `session-2026-06-11-001`, `session-2026-06-11-002`, `session-2026-06-21-001`, `session-2026-06-21-002`, `session-2026-06-29-001`, `session-2026-07-03-001`, `session-2026-07-03-002`, `session-2026-07-03-003`, `session-2026-07-03-004`, `session-2026-07-05-001`, `session-2026-07-05-002`, `session-2026-07-05-003`, `session-2026-07-05-004`, `session-2026-07-05-005`, `session-2026-07-05-006`, `session-2026-07-05-007`, `session-2026-07-05-008`, `session-2026-07-05-009`, `session-2026-07-05-010`, `session-2026-07-05-011`, `session-2026-07-05-012`, `session-2026-07-05-013`, `session-2026-07-05-014`, `session-2026-07-05-015`.
...
| 31 | `opencode.json`'s configured model (`opencode/big-pickle`) doesn't match the Capability Baseline's documented OpenCode model (`gemma4:e4b`) — flagged repeatedly during today's architecture review but never given its own tracked entry until now (2026-07-05) | 9 | Open — genuinely unresolved, not just undecided: the 2026-07-03 live-test session log (`session-2026-07-03-002.md`) itself records OpenCode's model as `big-pickle`, while the same project's own toolchain notes separately describe it as `gemma4:e4b` — these may not actually conflict (`big-pickle` could be OpenCode's internal routing name for the same underlying model) or one of the two records may simply be wrong. Neither Cowork nor Claude Code can resolve this without Cameron confirming what OpenCode is actually configured to run | Cameron to clarify |
| 32 | Two overlapping Draft-Agent-side manual session-start prompts exist without referencing each other — `Protocols/session-start-verify.md` and `Prompts/prompt-cowork-session-start-check.md`. Flagged repeatedly during today's architecture review but never given its own tracked entry until now (2026-07-05) | 9 | Open — not a bug, a documentation choice: merge the two into one, add cross-references so each points to the other, or leave both as legitimate independent alternatives. No option is obviously correct without knowing whether Cameron actually uses both or has settled on one in practice | Cameron decides |
